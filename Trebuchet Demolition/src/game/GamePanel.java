...
/**
 * A panel which draws all the shapes and contains a physics engine, and
...
public class GamePanel extends JPanel
{
	...
	public static Image metalTexture, brickTexture, rockTexture, debugTexture,
			ingameTexture;
	...
	public GamePanel(final int width, final int height)
	{
		...
		MenuItemAction helpMenuItemAction = new MenuItemAction() {
			...
			public void doAction(MenuItem item)
			{
				String help = "<html><h2>Welcome to Trebuchet Demolition!</h2>"
						+ "The goal of this game is to destroy a \"target\" (aka bricks) in a level, in order to complete the level.<br>"
						+ "The game employs physics, so you can bounce objects off walls and such.<br><br><hr>"
						+ "<b>Controls:</b><br>"
						+ "<table style=\"width:100%\"><tr><td>UP DOWN</td><td>Fine control angle</td></tr>"
						+ "<tr><td>LEFT RIGHT</td><td>Fine control power</td>"
						+ "<tr><td>ESC</td><td>Open pause menu</td>"
						+ "<tr><td>SCROLLWHEEL</td><td>Coarse control power</td>"
						+ "<tr><td>CTRL + SCROLLWHEEL</td><td>Coarse control angle</td></table><br><hr>"
						+ "<b>Scoring:</b><br>"
						+ "The game has a scoring system, based on a \"lower score is better\" policy.<br>"
						+ "Your score is determined by the amount of time it takes for you to reach the goal,<br>"
				...
				JOptionPane.showMessageDialog(null, help,
						"Trebuchet Demolition Help", JOptionPane.PLAIN_MESSAGE);
			}
		};
		MenuItemAction aboutMenuItemAction = new MenuItemAction() {
			...
			public void doAction(MenuItem item)
			{
				JOptionPane
						.showMessageDialog(
								null,
								"<html>Created by <b><i>Gordon Guan</b></i><br>(c) 2015<br>Block textures from http://webtreats.mysitemyway.com/<br>Music is The Blue Danube Waltz by Johann Strauss II<br><a href=http://github.com/Netdex/Trebuchet-Demolition>GitHub Repository</a></html>",
								"About Trebuchet Demolition",
		...
		int sep = 10;
		int shadowDist = 3;
		MenuItem playMenuItem = new MenuItem("PLAY", GraphicsTools.MAIN_FONT,
				Color.WHITE, GraphicsTools.BG_COLOR, new Rectangle(10, y,
						mlength, mheight), playMenuItemAction, shadowDist);
		MenuItem levelEditMenuItem = new MenuItem("LEVEL EDITOR",
				GraphicsTools.MAIN_FONT, Color.WHITE, GraphicsTools.BG_COLOR,
				new Rectangle(10, y + mheight + sep, mlength, mheight),
				levelEditMenuItemAction, shadowDist);
		MenuItem optionsMenuItem = new MenuItem("OPTIONS",
				GraphicsTools.MAIN_FONT, Color.WHITE, GraphicsTools.BG_COLOR,
				new Rectangle(10, y + mheight * 2 + sep * 2, mlength, mheight),
				optionsMenuItemAction, shadowDist);
		MenuItem helpMenuItem = new MenuItem("HELP", GraphicsTools.MAIN_FONT,
				Color.WHITE, GraphicsTools.BG_COLOR, new Rectangle(10, y
						+ mheight * 3 + sep * 3, mlength, mheight),
				helpMenuItemAction,
				shadowDist);
		MenuItem aboutMenuItem = new MenuItem("ABOUT", GraphicsTools.MAIN_FONT,
				Color.WHITE, GraphicsTools.BG_COLOR, new Rectangle(10, y
						+ mheight * 4 + sep * 4, mlength, mheight),
				aboutMenuItemAction, shadowDist);
		MenuItem exitMenuItem = new MenuItem("EXIT", GraphicsTools.MAIN_FONT,
				Color.WHITE, GraphicsTools.BG_COLOR, new Rectangle(10, y
						+ mheight * 5 + sep * 5, mlength, mheight),
				exitMenuItemAction,
				shadowDist);
		...
		musicMenuItem = new ToggleMenuItem("Music", GraphicsTools.OPTIONS_FONT,
				Color.WHITE, Color.GREEN, Color.RED, new Rectangle(10, 75, 500,
						40), musicMenuItemAction, 2);
		MenuItem returnMenuItem = new MenuItem("Return to Main Menu",
				GraphicsTools.SMALL_TEXT_FONT, Color.WHITE,
				GraphicsTools.BG_COLOR, new Rectangle(10, 125, 500, 27),
				returnMenuItemAction, 1);
		...
		MenuItem resumeMenuItem = new MenuItem("Resume",
				GraphicsTools.OPTIONS_FONT, Color.WHITE,
				GraphicsTools.BG_COLOR, new Rectangle(110, 170, 550, 40),
				resumeMenuItemAction, 2);
		MenuItem returnFromPauseMenuItem = new MenuItem("Return to Main Menu",
				GraphicsTools.OPTIONS_FONT, Color.WHITE,
				GraphicsTools.BG_COLOR, new Rectangle(110, 230, 550, 40),
				returnMenuItemAction,
				2);
		...
		this.addMouseMotionListener(new MouseMotionListener() {
			...
			{
				// Tell the menus to highlight certain menu items if they are
				// mouse overed
				if (displayScreen == ScreenType.MAIN_MENU)
				{
	...
	/**
	 * Deletes all the items in the level select menu and loads them all from
	 * LevelManager
	 */
	public void repopulateLevelSelectMenu()
	{
		...
		for (int levelID = 0; levelID < LevelManager.getLevels().size(); levelID++)
		{
			...
			MenuItemAction levelSelectAction = new MenuItemAction() {
				...
				public void doAction(MenuItem item)
				{
					Level level = LevelManager.getLevels().get(
							levelSelectMenu.getMenuItems().indexOf(item));
					loadLevel(level);
			...
			{
				highscore = "Highscore: "
						+ level.getMetadata().getProperty("highscore");
			}
			String menuItemText = String.format("%-25s", level.getName()
					.substring(0, Math.min(level.getName().length(), 40)))
					+ String.format("%-20s", level.getFile().getName())
					+ String.format("%20s", highscore);
			MenuItem menuItem = new MenuItem(menuItemText,
					GraphicsTools.LEVEL_SELECT_FONT, Color.WHITE,
					GraphicsTools.BG_COLOR.darker().darker(), new Rectangle(10,
							y, mlength, mheight),
					levelSelectAction, 1);
		...
		levelSelectMenu.addMenuItem(new MenuItem("Return to Main Menu",
				GraphicsTools.SMALL_TEXT_FONT, Color.WHITE,
				GraphicsTools.BG_COLOR, new Rectangle(10, y, mlength, mheight),
				new MenuItemAction() {
					@Override
					public void doAction(MenuItem item)
					{
	...
	public void loadLevel(Level level)
	{
		try
		{
			...
			// String[] colorRGB =
			// level.getMetadata().getProperty("bgcolor").replaceAll(" ",
			// "").split(",");
			// Color color = new Color(Integer.parseInt(colorRGB[0]),
			// Integer.parseInt(colorRGB[1]), Integer.parseInt(colorRGB[2]));
		...
		catch (Exception e)
		{
			JOptionPane
					.showMessageDialog(
							null,
							"An error occured while loading a level: "
									+ e.getMessage(), "Level Loading Error",
							JOptionPane.ERROR_MESSAGE);
			TrebuchetDemolition.LOGGER
					.warning("Failed to load the level selected: "
							+ e.getMessage());
			return;
		}
		loadedLevel = level;
	...
	public void checkWin()
	{
		if (engine.hasWon())
		{
			String winMessage = "You have won with a score of "
					+ loadedLevel.getScore() + "!";
			// Highscore checking
			String highscore = loadedLevel.getMetadata().getProperty(
					"highscore");
			if (highscore == null)
			{
			...
			else if (Integer.parseInt(highscore) > loadedLevel.getScore())
			{
				...
				winMessage += " You beat the highscore of " + highscore
						+ " by " + diff + " points!";
			}
			JOptionPane.showMessageDialog(null, winMessage, "Level Clear",
					JOptionPane.INFORMATION_MESSAGE);
			stop();
			engine.clearAll();
			...
			{
				TrebuchetDemolition.LOGGER.warning("Failed to load entities: "
						+ e.getMessage());
			}
			loadedLevel = null;
			lastFirePower = 0;
	...
	/**
	 * Calculates and sets the score based on a predetermined algorithm based on
	 * time and power
	 */
	public void doScore()
	{
		if (loadedLevel != null)
		{
			if (lastFireTime == -1) // Projectile never fired
				...
				loadedLevel
						.setScore((int) ((System.currentTimeMillis() - lastFireTime)
								* lastFirePower * lastFirePower) / 100000);
		}
	}
	...
	public void paintComponent(Graphics graphics)
	{
		...
		if (displayScreen == ScreenType.IN_GAME)
		{
			...
			g.drawImage(ingameTexture, 0, 0, width, height, 0, 0,
					ingameTexture.getWidth(null),
					ingameTexture.getHeight(null), null);
			...
			GraphicsTools.drawShadowedText(g, loadedLevel.getScore() + "",
					scoreX, scoreY, 2);
			GraphicsTools.drawShadowedText(g, loadedLevel.getScore() + "",
					scoreX, scoreY, -2);
			/* End score drawing */

			g.drawImage(trebuchetImage, 20, height - 60, 80, height, 0, 0,
					trebuchetImage.getWidth(null),
					trebuchetImage.getHeight(null), null);
			...
			g.setColor(Color.RED);
			g.drawLine((int) shootPoint.x, (int) shootPoint.y,
					(int) (shootPoint.x - vel.x), (int) (shootPoint.y - vel.y));
			g.setColor(Color.BLACK);
			...
			if (paused)
			{
				...
				g.fillRect(100, 100, width - 200, g.getFontMetrics()
						.getAscent() + 10);
				g.setColor(Color.WHITE);
		...
		else if (displayScreen == ScreenType.MAIN_MENU)
		{
			...
			g.drawImage(backgroundImage, 0, 0, backgroundImage.getWidth(null),
					backgroundImage.getHeight(null), 0, 0, width, height, null);
	...
	public static void loadResources()
	{
		try
		{
			titleImage = ImageIO.read(TrebuchetDemolition.class
					.getResourceAsStream("/resources/title.png"));
			backgroundImage = ImageIO.read(TrebuchetDemolition.class
					.getResourceAsStream("/resources/background.jpg"));
			trebuchetImage = ImageIO.read(TrebuchetDemolition.class
					.getResourceAsStream("/resources/trebuchet.png"));
			metalTexture = ImageIO.read(TrebuchetDemolition.class
					.getResourceAsStream("/resources/metal.jpg"));
			ingameTexture = ImageIO.read(TrebuchetDemolition.class
					.getResourceAsStream("/resources/ingamebg.jpg"));
			brickTexture = ImageIO.read(TrebuchetDemolition.class
					.getResourceAsStream("/resources/brick.jpg"));
			rockTexture = ImageIO.read(TrebuchetDemolition.class
					.getResourceAsStream("/resources/wood.jpg"));
			debugTexture = ImageIO.read(TrebuchetDemolition.class
					.getResourceAsStream("/resources/debug.jpg"));
			TrebuchetDemolition.LOGGER.info("Loaded resources");
		}
		catch (Exception e)
		{
			TrebuchetDemolition.LOGGER.severe("Failed to load game resources: "
					+ e.getMessage());
		}
	}
