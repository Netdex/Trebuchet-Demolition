...
public class Menu
{
	...
	/**
	 * Makes the menu respond to the given MouseEvent to do actions defined in
	 * the given MenuItemAciton
	 * 
	...
	public void handleHighlights(MouseEvent event)
	{
		...
		// Loop through every menu item to see if the mouse is inside one
		for (MenuItem item : this.getMenuItems())
		{
			if (item.getDimensions().contains(p))
			{
